...
const get_all_patients = async (req, res, next) => {
    ...
    try {
        ...
        const patient_ref = db.collection(`users/${req.authId}/patient_records`)
        ...
        for (const doc of patient_snapshot.docs) {
            ...
            const path = `users/${req.authId}/patient_records/${doc.id}/screeningResults`;
...
const get_single_patient = async(req, res, next) => {
    ...
    try {
        ...
        if (patient_snapshot.empty){
            return res.status(400).json({msg : "Patient doesn't exist!"})
        }
